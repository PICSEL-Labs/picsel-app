      ...
      return (
        <Pressable
          ...
          <View style={defaultShadow} className="mb-[7px] rounded-full">
            <View className="h-[60px] w-[60px] items-center justify-center overflow-hidden rounded-full">
              <Image
                source={{ uri: Config.IMAGE_URL + brand.iconImageUrl }}
                className="h-[60px] w-[60px] rounded-full"
                resizeMode="cover"
                style={
                  isRemoveMode && isSelected ? { opacity: 0.4 } : undefined
                }
              />
  ...
  return (
    <ScreenLayout>
      <MypageHeader
        ...
        rightElement={<Kebab />}
        rightIconPress={handleToggleMenu}
      />
      ...
      {isRemoveMode && (
        <View className="flex-row px-4 pb-2 pt-3">
          <Pressable
            className="ml-2 flex-1 flex-row items-center justify-center rounded-lg py-3"
